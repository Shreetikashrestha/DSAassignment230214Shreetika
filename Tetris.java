...
// //Initialization
// Create a 2D board (20x10) to store blocks.
// Use a queue to manage upcoming blocks.
// Generate and enqueue the first random block.
// Start a timer to control block movement.
// Game Loop (Repeats Until Game Over)
// Check for game over (if a new block can’t be placed).
// Display board and upcoming block.
// Handle user input (left, right, down, rotate).
// Move block down if space is available.
// If a collision occurs:
// Place block on board.
// Check for full rows and clear them (shift rows down).
// Generate new block.
// Block Rotation & Movement
// Check if the move is valid (collision detection).
// If valid, update block position or rotate it.
// Game Over
// If no new block can be placed, game ends.
