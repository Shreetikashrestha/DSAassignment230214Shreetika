...
// Output:2
// Return the total number of roads traversed, ensuring roads passed multiple times are counted accordingly.


// Input:
// packages[]: An array where each index represents a location, and value 1 indicates a package at that location.
// roads[][]: A 2D array representing undirected roads between locations.


// Graph Construction:
// Create an adjacency list graph where each location maps to its connected neighbors.


// Identify Package Locations:
// Iterate through the packages array and collect indices where the value is 1. These are the package locations.


// Breadth-First Search (BFS):
// Use BFS to find the shortest path (in terms of roads) between:
// Start location (0) and each package location.
// Each package location and the next.
// The last package location and the start location.


// Distance Calculation:
// For each package location, calculate the distance from the current location.
// Sum all these distances and include the distance to return to the start location.
